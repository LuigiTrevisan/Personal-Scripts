...
def next_guess(all_combinations):
    ...
    for guess in all_combinations:
        ...
        for feedback in itertools.product(range(num_pegs + 1), repeat=2):
            count = len([comb for comb in all_combinations if get_feedback(guess, comb) == feedback])
...
print(Back.GREEN + Fore.BLACK + "Developed by: @LuigiTrevisan\n" + Style.RESET_ALL)
...
while True:
    ...
    all_combinations = filter_combinations(all_combinations, current_guess, (black, white))
